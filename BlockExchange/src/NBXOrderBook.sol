...
contract NBXOrderBook is Ownable, ReentrancyGuard {
    ...
    // Events
    event OrderCreated(uint256 indexed orderId, address indexed maker, address indexed tokenAddress, uint256 amount, uint256 price, OrderType orderType);
    event OrderFilled(uint256 indexed orderId, address indexed maker, address indexed taker, uint256 amount, uint256 price);
    ...
    function _matchSellOrder(Order storage sellOrder) internal {
        ...
        for (uint256 i = 0; i < tokenBuyOrders.length && sellOrder.filledAmount < sellOrder.amount; i++) {
            ...
            uint256 remainingSellAmount = sellOrder.amount - sellOrder.filledAmount;
            uint256 remainingBuyAmount = buyOrder.amount - buyOrder.filledAmount;
            uint256 matchAmount = remainingSellAmount < remainingBuyAmount ? 
